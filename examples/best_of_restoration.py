"""Full best-of continuations with initially violated upper constraints.
...
def main() -> None:
    ...
    problem = BilevelProblem(
        ...
        lower,
        # Some sampled points violate this and invoke feasibility restoration.
        outer_constraints=[x + y >= 1.5],
    )
    result = problem.solve(best_of=5, seed=23, restoration=True)

    print(f"status: {result.status}")
    print(f"successful runs: {sum(run.succeeded for run in result.runs)}")
